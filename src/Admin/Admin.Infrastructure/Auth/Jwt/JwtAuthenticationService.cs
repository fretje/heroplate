...
public class JwtAuthenticationService : AuthenticationStateProvider, IAuthenticationService, IAccessTokenProvider
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        {
            claimsIdentity.AddClaims(cachedPermissions.Select(p => new Claim(CustomClaimTypes.Permission, p)));
        }
    ...
    public void NavigateToLogin(string? returnUrl = null) =>
        _navigation.NavigateToLogin("login", new InteractiveRequestOptions { Interaction = InteractionType.SignIn, ReturnUrl = returnUrl });
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private async Task<(bool Succeeded, TokenResponse? Token)> TryRefreshTokenAsync(RefreshTokenRequest request)
    {
        ...
        try
        {
            ...
            await CacheAuthTokensAsync(tokenResponse.Token, tokenResponse.RefreshToken);
    ...
    private async ValueTask CacheAuthTokensAsync(string? token, string? refreshToken)
    {
        await _localStorage.SetItemAsync(AuthTokenKey, token);
        await _localStorage.SetItemAsync(RefreshTokenKey, refreshToken);
    }

    private ValueTask CachePermissionsAsync(ICollection<string> permissions) =>
        _localStorage.SetItemAsync(PermissionsKey, permissions);

    private async Task ClearCacheAsync()
    {
        await _localStorage.RemoveItemAsync(AuthTokenKey);
        await _localStorage.RemoveItemAsync(RefreshTokenKey);
        await _localStorage.RemoveItemAsync(PermissionsKey);
    }

    private ValueTask<string> GetCachedAuthTokenAsync() =>
        _localStorage.GetItemAsync<string>(AuthTokenKey);
    ...
    private ValueTask<ICollection<string>> GetCachedPermissionsAsync() =>
        _localStorage.GetItemAsync<ICollection<string>>(PermissionsKey);
