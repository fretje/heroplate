...
internal class AzureAdJwtBearerEvents : JwtBearerEvents
{
    ...
    public override async Task TokenValidated(TokenValidatedContext context)
    {
        ...
        // Lookup the tenant in the cache using the issuer.
        var tenant = await context.HttpContext.RequestServices.GetRequiredService<ITenantCache>()
            .GetByIssuerAsync(issuer);
        ...
        // Adding tenant claim.
        identity.AddClaim(new Claim(CustomClaimTypes.Tenant, tenant.Id));

        // Set new tenant info to the HttpContext so the right connectionstring is used to lookup the user.
        context.HttpContext.TrySetTenantInfo(tenant, true);

        // Lookup local user or create one if none exist.
        var createUserService = context.HttpContext.RequestServices.GetRequiredService<ICreateUserService>();
        string userId = await createUserService.GetOrCreateFromPrincipalAsync(principal);
        ...
        identity.AddPermissions(
            await userCache.GetPermissionsAsync(userId));
