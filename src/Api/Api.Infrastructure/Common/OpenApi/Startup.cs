    ...
    internal static IServiceCollection AddOpenApiDocumentation(this IServiceCollection services, IConfiguration config)
    {
        ...
        if (settings?.Enable is true)
        {
            ...
            services.AddOpenApiDocument((document, serviceProvider) =>
            {
                ...
                if (config["SecuritySettings:Provider"]?.Equals("AzureAd", StringComparison.OrdinalIgnoreCase) is true)
                {
                    document.AddSecurity(JwtBearerDefaults.AuthenticationScheme, new OpenApiSecurityScheme
                    {
                        ...
                        Flows = new()
                        {
                            AuthorizationCode = new()
                            {
                                ...
                                {
                                    { config["SecuritySettings:Swagger:ApiScope"] ?? throw new InvalidOperationException("Swagger ApiScope not specified in SecuritySettings."), "access the api" }
                                }
                            }
                        }
                    });
